...
function EditarPoliticaCancelamento() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        const data = await politicaCancelamentoService.buscarPorId(id);
        setForm({
          descricao: data.descricao,
          prazoMinimoDias: data.prazoMinimoDias,
          percentualReembolso: data.percentualReembolso,
        });
      } catch {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const politica = new PoliticaCancelamento({ id });
      politica.editar({
      ...
      await politicaCancelamentoService.atualizar(id, politica.toJSON());
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Editar Política de Cancelamento</h1>
...
export default EditarPoliticaCancelamento;
